...
<script type="text/javascript">
	...
	function deleteUser(id,obj) {
		if (id == "${USER_INFO.userId}") {
			...
			if (confirm("您真的要删除该员工吗？")) {

				xmlAjaxRequest("deleteuser.order?userId=" + id + "&time="
						+ Math.random(), "get", true, null, deleteCallback,
						obj, null);
			}
		}
		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				...
				<ul class="nav nav-sidebar">
					...
					<li  class="active"><a href="touseradmin.order?page=1"><i
							class="icon-group icon-large"></i>&nbsp;员工管理</a></li>
					<li><a href="toadminmain.order?page=1"><i
							...
							<table class="table table-striped">
								<thead>
									<tr>
										...
										<th>员工帐号</th>
										<th>员工角色</th>
										<th colspan="3" style="width:5%;"><center>操作</center></th>
									</tr>
								</thead>
							 	<tbody>
									...
									<tr>
										...
										<td><c:choose><c:when test="${temp.role==1}">管理员</c:when><c:otherwise><c:choose><c:when test="${temp.role==2}">后厨</c:when><c:otherwise>服务员</c:otherwise></c:choose></c:otherwise></c:choose></td>
										
										
										<td><a href="tomodifyuser.order?userId=${temp.userId}"><span class="icon-cogs" aria-hidden="true"></a></td>
										<td><div onclick="deleteUser(${temp.userId},this)"><i class="icon-remove-sign" aria-hidden="true"></i></div></td>
							
<!-- onclick="deleteUser(${temp.userId},${temp.userAccount},this) -->
										<td>
											<a href="" data-toggle="modal"
											data-target=".bs-dishes${temp.userId}-modal-sm"><span class="icon-sitemap" aria-hidden="true"></a>
												
												 <!-- 查看菜的信息 -->
											<div class="modal fade bs-dishes${temp.userId}-modal-sm" tabindex="-1"
														...
														<div class="panel panel-success" style="margin: 15px 0px;">
															...
															<div class="panel-body" style="padding: 20px 0px;">
																<div style="width: 200px; margin: auto;">
																	...
																	<div class="panel-heading">
																		<img src="" style="border: grey 5px solid; width: 100px;"></img>
																		<h2 class="panel-title" style="font-size: 16px;">员工账号：${temp.userAccount}</h2>
																		<h2 class="panel-title" style="font-size: 16px;">员工角色：<c:choose><c:when test="${temp.role==1}">管理员</c:when><c:otherwise><c:choose><c:when test="${temp.role==2}">后厨</c:when><c:otherwise>服务员</c:otherwise></c:choose></c:otherwise></c:choose></h2>
							...
							<nav>
							<ul class="pager">
								<li><a href="touseradmin.order?page=1" id="firstpage">&larr;首页</a></li>
								<li><a href="touseradmin.order?page=${page-1} " id="pre">上一页</a></li>
								<li><a href="touseradmin.order?page=${page+1 }" id="next">下一页</a></li>
								<li><a href="touseradmin.order?page=-1" id="last">末页&rarr;</a></li>
							</ul>
							</nav>
